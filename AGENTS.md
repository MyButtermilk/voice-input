...
- Final transcript insertion policy (duplicate-proof):
  - Only the Accessibility Service is used to insert the final text (IME direct commit and clipboard fallback are disabled).
...
- Always reinstall after modifying Soniox or accessibility code (`./gradlew installDevDebug`) so the updated logic takes effect on-device.
